$(function(){
  ...
  (function deleteNotNecesaryBillingAddress(){
    getAll('prospects', false).then(prospects => {

      let billingAddressIds = prospects.map(prospect => {
        return prospect.billing_address_id;
      });

      let finalQuery = 'DELETE FROM billing_addresses' +
        ` WHERE id NOT IN (${billingAddressIds})`;
