  ...
  findBy('id', serviceOffered.service_id, 'services').then(serviceObject => {
    ...
    findBy('service_offered_id', serviceOffered.id, 'delivery_services').then(deliveryService => {
      ...
      deliveryCad += `${deliveryServiceObject.tracking_number}` +
        ...
        '</tr>' +
        '<tr>' +
        '<td style="text-align:left"> Dir. destino: </td>' +
        '<td colspan="3" style="text-align:right">' +
  ...
  createTicketProductList(ticketData.products, ticketData.ticket.discount_applied, function(productsCad, deliveriesCad){
    createPaymentFormList(ticketData.payments, function(paymentsCad){
      return call('<!DOCTYPE html>' +
        '<html lang="es" style="width: 65mm !important">' +
        '<head>' +
        '<meta charset="UTF-8">' +
        '<title> Ticket </title>' +
        '<style type="text/css">' +
        '@media print{' +
        '@page { size:65mm auto !important; width: 65mm; margin-left: 0mm !important; margin-right: 0mm !important}' +
          'html,body {' +
            'margin: 0 !important; width: 65mm !important; padding: 0 !important;' +
            '}' +
        '}' +
        '</style>' +
        '</head>' +
        '<body style="margin:0 auto !important; padding:0 auto !important; width: 65mm !important">' +
        '<table style="font-family: Arial; font-size: 11px; width: 65mm; !important; margin-left: 0mm !important; margin-right: 0mm !important; text-align: center; vertical-align:text-top">' +
        '<tbody>' +
        '<tr>' +
        '<td colspan="4">' +
